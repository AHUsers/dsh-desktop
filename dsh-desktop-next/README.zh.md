...
- **首次引导：** 每个尚无完成记录的 Profile 会在 Host 启动前显示四页引导：欢迎、插件市场（社区市场、dsh-market 或暂不开启）、远程控制、恢复模式说明。布局与分组渐入动效参考 AA，支持浅色、深色和减少动态效果。左上角提供“上一步”，右上角每页均可“跳过全部”。返回保留未保存的选项；完成时将选择和完成记录一并写入当前 Profile 清单，跳过则保留原配置。已完成的 Profile 不再重复显示。恢复模式和隔离的安全模式直接进入，修复有效 Profile 时保留引导记录。
