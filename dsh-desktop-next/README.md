...
- **Markets:** manage `dsh-community-market` (on by default) and `dshmarket` (off by default) independently in the official **Plugins** page; both can run together. Community Market keeps its sidebar entry; dshmarket keeps **Settings → Plugin Market**. Community Market retains discovery, sources, installation previews, confirmation and removal. Package operations use the bundled pnpm. Completed operations can request a restart; the terminal action is available on macOS and Windows.
- **Remote control:** enable `@agents-anywhere/dsh-bridge-next` (off by default) in the official **Plugins** page, then configure it through the phone connection page in the official main interface. Connector state is scoped by Profile within Next home. Switching Profiles stops the previous Host and its remote connection.
...
The three bundled plugins use the official cards, bundle switches and component switches. They are shipped with Next, so they can be disabled but cannot be uninstalled from the application. Bundle plugins installed into a Profile by either market appear in the official manager for enablement, disablement and removal. dshmarket’s own plugin-disable behavior is unchanged. The previous Desktop settings selectors are hidden in Next. Existing choices migrate once from `desktop-next.features.json` into `package.json`; subsequent starts respect the official manager. Per-Profile remote connector paths remain unchanged.
...
Next is a profile bundle so shared plugin-manager reconciliation retains its capabilities. A version-scoped plugin-manager patch reads the installation manifest’s `dsh.optionalBundles`, exposing shipped optional bundles through the unchanged official UI even when disabled. The dshmarket package runner uses the official `runPluginCommand` operation with Next’s Profile, installation anchor and bundled pnpm; it does not require pnpm on the system PATH. Development startup creates one managed link for Next itself under `home/profiles/node_modules`; alpha.2 runtime resolution owns all other dependency fallbacks. All upstream runtime dependencies come from published packages, without source links into or edits to `deepseek-harness/`.
