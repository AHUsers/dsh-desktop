...
export function DesktopPermissionsSection({ service, language }: { service: DesktopPermissions; language: string }) {
  ...
  useEffect(() => {
    if (location.hash === '#permissions') section.current?.scrollIntoView({ block: 'start' })
  }, [snapshots.length])
  useEffect(() => {
    let disposed = false
    const refresh = (): void => {
  ...
  return <section ref={section} className="dshDesktopSettingsGroup" aria-label={t('系统权限', 'System permissions')}>
